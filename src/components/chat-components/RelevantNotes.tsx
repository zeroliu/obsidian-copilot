...
import { ArrowRight, ChevronDown, ChevronUp, PlusCircle, TriangleAlert } from "lucide-react";
...
function useHasIndex(notePath: string) {
  ...
  ({ children, ...props }, ref) => {
    return (
      <div ref={ref} {...props} className="flex gap-2">
        <Badge variant="secondary" className="text-xs text-muted whitespace-nowrap cursor-default">
          {children}
        </Badge>
      </div>
    );
    ...
    <div className="flex gap-2 p-2 justify-between rounded-md border border-border border-solid">
      <div className="flex flex-col gap-1 flex-1 overflow-hidden">
        <a
          ...
          className="text-sm text-normal text-ellipsis overflow-hidden whitespace-nowrap w-full"
        >
          {note.document.title}
        </a>
        {showPath && (
          <span className="text-xs text-muted text-ellipsis overflow-hidden whitespace-nowrap w-full">
            {note.document.path}
          </span>
        )}
        <div className="flex gap-1">
...
export function RelevantNotes({
  ...
  const relevantNotes = useRelevantNotes();
  const activeFile = app.workspace.getActiveFile();
  const hasIndex = useHasIndex(activeFile?.path ?? "");
